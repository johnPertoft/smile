...
class AttGAN(Model):
    def __init__(self,
        ...
        # Note: Target attributes for static samples are generated by for each attribute inverting the corresponding
        # attribute in each sample's actual attribute vector.
        ...
        attributes_test_static_repeated = tf.reshape(tf.tile(attributes_test_static, [1, n_classes]), [-1, n_classes])
        eye_repeated = tf.tile(tf.eye(n_classes), [n_samples, 1])
        x_test_static_target_attributes = tf.logical_xor(
            tf.cast(eye_repeated, tf.bool),
            tf.cast(attributes_test_static_repeated, tf.bool))
        x_test_static_target_attributes = tf.cast(x_test_static_target_attributes, tf.float32)
